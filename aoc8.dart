...
List<List<List<String>>> parse(String s) {
  return s
      .split('\n')
      .map((line) =>
          line.split(' | ').map((block) => block.split(' ').toList()).toList())
      .toList();
}
...
good(List<String> l, List<String> pNums) {
  for (var el in l) {
    if (!pNums.contains(el)) {
      return false;
    }
  }
  return true;
...
var letters = 'abcdefg';
var perms = Permutations(letters.length, characters(letters))().map((chars) {
  var map = <String, String>{};
  var mapReverse = <String, String>{};
  for (var i = 0; i < chars.length; i++) {
    map[chars[i]] = letters[i];
    mapReverse[letters[i]] = chars[i];
  }
...
var nums = numsRaw.map((n) => n.split('').toList()).toList();

calc2(String s) {
  var lines = parse(s);
  var finals = lines.map((l) {
    var all = l.expand((element) => element).map((n) {
      var z = n.split('').toList();
      z.sort();
      return z.join();
    }).toList();
    for (var perm in perms) {
      ...
      var pNums = nums.map((n) {
        var z = n.map((i) => p[i]).toList();
        z.sort();
        return z.join();
      }).toList();
      if (!good(all, pNums)) {
      ...
      var out = l[1];
      var decoded = out.map((n) {
        var z = n.split('').map((i) => pReverse[i]).toList();
        z.sort();
        return z.join();
      }).toList();
      var finalNums = decoded.map((n) => numsRaw.indexOf(n)).toList();

      return finalNums;
    }
    return [-1];
  });
  return finals.map((l) => l.join());
}

aoc8() {
  // return calc1(input);
  return calc2(input).map(int.parse).reduce((a, b) => a + b);
}
